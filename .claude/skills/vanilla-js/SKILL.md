...
**Баг в JS:** сначала тест в `websocket.test.js` / `ui.test.js` (красный) → потом фикс → тест зелёный.
**Новый JS-обработчик:** сначала тест → потом код.

```bash
npm test -- --testNamePattern="имя теста"   # проверить конкретный тест
npm test                                     # регрессия
```

Нельзя менять `ui.js` / `websocket.js` без соответствующего теста, написанного ДО изменений.
